...
class Slider_Mosaic_Widget extends WP_Widget {
    ...
    public function form($instance) {
        ...
        isset($instance['criterio']) ? $criterio = $instance['criterio'] : '';
        ?>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('criterio'); ?>"><?php _e('Critério:'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('criterio'); ?>" name="<?php echo $this->get_field_name('criterio'); ?>" type="text" value="<?php echo esc_attr($criterio); ?>">
        </p>
    ...
    public function update($new_instance, $old_instance) {
        ...
        $instance['criterio'] = (!empty($new_instance['criterio']) ) ? strip_tags($new_instance['criterio']) : '';
    ...
    public function widget($args, $instance) {
        ...
        $html = '';
        $html .= '<section class="zerogrid-section" criterio="'.$criterio.'" style="display:none;">';
        ...
        while ( $loop->have_posts() ) : 
            ...
            if(!empty(get_the_terms($post->ID,"criterio_slider")))
                foreach(get_the_terms($post->ID,"criterio_slider") as $term)
                    if($criterio == $term->slug)
                        array_push($postsDaCidade,$post->ID);
        endwhile;

        if(sizeof($postsDaCidade)>0 || $criterio == ""):
            while ( $loop->have_posts() ) : 
                $loop->the_post();
                global $post;
                ...
                if(!empty($criterio))
                    if(!in_array($post->ID,$postsDaCidade)) continue;
            ...
            ${"quadro" . $custom["_Z_slider_ordem"][0]}  = '<div class="wrap-col criterioMosaico">';
